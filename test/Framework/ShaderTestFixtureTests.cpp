...
SCENARIO("HLSLFrameworkTests - AssertBuffer - ResultProcessing - AssertInfoWithData")
{
    ...
    auto serialize = []<typename T>(const T& InVal)
    {
        std::vector<std::byte> ret;
        static constexpr u32 size = sizeof(T);
        ret.resize(sizeof(u32) + size);
        std::memcpy(ret.data(), &size, sizeof(T));
        std::memcpy(ret.data() + sizeof(u32), &InVal, size);
        return ret;
    };

    static constexpr u32 expectedValue = 34u;
        ...
        (
            {
                ...
                std::tuple
                {
                    ...
                    STF::TestRunResults{ {STF::FailedAssert{serialize(expectedValue), {}, STF::AssertMetaData{42, 0, 0}}}, 0, 1, uint3(1,1,1)},
                    10, 400
                },
                std::tuple
                {
