...
template <typename T>
struct std::formatter<stf::HLSLVector<T, 2>> : std::formatter<string_view> {
    auto format(const stf::HLSLVector<T, 2>& In, auto& ctx) const {
        std::string temp;
...
template <typename T>
struct std::formatter<stf::HLSLVector<T, 3>> : std::formatter<string_view> {
    auto format(const stf::HLSLVector<T, 3>& In, auto& ctx) const {
        std::string temp;
        std::format_to(std::back_inserter(temp), "({}, {}, {})",
...
template <typename T>
struct std::formatter<stf::HLSLVector<T, 4>> : std::formatter<string_view> {
    auto format(const stf::HLSLVector<T, 4>& In, auto& ctx) const {
        std::string temp;
        std::format_to(std::back_inserter(temp), "({}, {}, {}, {})",
